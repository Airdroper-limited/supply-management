...
class _ActiveTripState extends State<ActiveTrip> {
  ...
  List<Marker> avaliableAirdroperMarkers = List.empty(growable: true);
          ...
          children: [
            Expanded(
              ...
              markers: {
                ...
                if (trip.isActive && trip.airdroperMarkerLatLng != null)
                  Marker(
                    icon: AssetLoaderProvider.of(context).markerIconAirdroper,
                    position: TripProvider.of(context).airdroperMarkerLatLng!,
                    //smooth marker update does not work on flutter web
                    markerId: MarkerId('airdroperMarker ' +
                        (kIsWeb ? DateTime.now().toIso8601String() : '')),
                  )
              },
